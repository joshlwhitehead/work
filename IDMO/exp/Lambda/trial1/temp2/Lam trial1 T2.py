# -*- coding: utf-8 -*-
"""
Created on Mon Mar  1 13:52:25 2021
...
def amp_plot():
    ...
    plt.title('Amplification Curve (Trial 1)',size=30)
...
def melt_plot():
    ...
    plt.title('Melting Curve (Trial 1)',size=30)
    plt.tick_params(labelsize=30)
    plt.legend(prop={'size':20})
    plt.grid()
    plt.show()
...
def deriv_plot():
    ...
    plt.xlabel('Temperature (C)',size=30)
    plt.title('Melting Peak (Trial 1)',size=30)
    plt.tick_params(labelsize=30)
    plt.legend(prop={'size':20})
    plt.grid()
    plt.show() 
