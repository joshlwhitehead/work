...
def parsPCRTxt(file):                                                                                                       #read PCR data
    ...
    for u in file:
        ...
        if start and 'DATAQ:' in u or start and 'CHUBE:' in u:                                                                                     #only collect temps in these lines
            ...
            try:
                if heatCollect and len(heatTherm[countH]) != 0:
                    heatTherm.append([])
                    timeHeatTherm.append([])
                    countHTherm+=1
                elif coolCollect and len(coolTherm[countC]) != 0:
                    coolTherm.append([])
            ...
            if heatCollect:                                                                                             #start collecting heating temps
                ...
                timeHeatTherm[countHTherm].append(float(file[countLines].split()[0].strip('()'))/1000)
                # totalTime.append(float(file[countLines-1].split()[0].strip('()'))/1000)
                ...
                timeCoolTherm[countCTherm].append(float(file[countLines].split()[0].strip('()'))/1000)
                # totalTime.append(float(file[countLines-1].split()[0].strip('()'))/1000)
    ...
    return (heat[:-1],timeH[:-1]),(cool[:-1],timeC[:-1]),(denatTemp,annealTemp),(totalTemp,totalTime),(heatTherm,timeHeatTherm),(coolTherm,timeCoolTherm)             #return heating temps and times, cooling temps and times, and the set temps for denature and anneal





print(parsPCRTxt('PThermo_AdvBuild13_w86_230306_run1.txt')[4][0])
