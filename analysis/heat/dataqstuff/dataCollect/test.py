...
def denature():                                                                                     #create function to analyze denature temp
    ...
    if checkVar0.get() == 1:
        ...
        plt.plot(probs,'o')                                                             #plot probability that temperature is found outside of limits
        plt.grid()
        plt.xlabel('AdvB')
...
def anneal():                                                                               #function to analyze anneal temps
    ...
    m_compMM = pairwise_tukeyhsd(endog=dfAnova['Mean'], groups=dfAnova['Instrument'], alpha=alpha)          #use tukey method to compare instruments
    m_compMult = pairwise_tukeyhsd(endog=dfTemp['Temp'], groups=dfTemp['Instrument'], alpha=alpha)          #use tukey method to compare runs
    print(m_compMM)
    print(m_compMult)
