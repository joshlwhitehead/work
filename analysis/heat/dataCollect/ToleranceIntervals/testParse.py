...
def parsPCRTxt(file):                                                                                                       #read PCR data
    ...
    for u in file:
        ...
        if 'goto' in u and 'Controlled' not in u and float(u.split()[-1]) > 85:                                         #only look for temps under these conditions for heating
            ...
            heatCollect = False
        elif 'COOL slow down->steady state' in u:
            coolCollect = True
            cool.append([])
            timeC.append([])
            countC+=1
            ...
            if heatCollect:                                                                                             #start collecting heating temps
                ...
                try:
                    ...
                    # totalTime.append(float(file[countLines+1].split()[0].strip('()'))/1000)
                                                                                                        ...
                                                                                                        #start collecting cooling temps
...
y = parsPCRTxt('data/adv32_tapeCup_envROOM_1.txt')[1][0]

import numpy as np
for i in y:
    print(np.mean(i))
# x = parsPCRTxt('dataC/adv22_pcr_w71_230519_run06.txt')[1][1]
# # print(y)
# # print(y)
# z = []
# for i in y:
